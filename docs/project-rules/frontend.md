...
所有组件都适用：需要扩展组件内容、props、事件或 slot 时，必须先查看组件已有能力和原有语义。

如果已有属性可以满足需求，优先复用已有属性，不新增同职责属性。

例如弹窗已有 `showFooter` 控制底部区域时，不再新增 `showActions` 等重复属性。

确实需要新增能力时，先给出修改内容、影响范围和调用方式，待确认后再修改文件。
