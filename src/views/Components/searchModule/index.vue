...
<script setup>import { ref, computed, onMounted } from 'vue' import { useFormStore } from '@/store/formation.js' import components from './index.js' import { Search, Refresh } from '@element-plus/icons-vue' import searchCriteria from './publicModules/searchCriteria.vue' import searchFormFn from '@views/Components/searchModule/public.js' const store = useFormStore() const emit = defineEmits(['setCallback']) const getComponent = (name) => components[name] const props = defineProps({ formData: { type: Array, default: () => [] }, width: { type: String, default: () => {} }, show: { type: Boolean, default: false }, keys: { type: String, default: () => 'public' } }) const ruleForm = ref({}) const loading = ref(false) const switchKG = ref(false) const onSubmit = () => { ruleForm.value = { ...store.getSearchRuleForm, page: 1, pagesize: 15 } emit('setCallback', ruleForm.value) } const onRefresh = () => { ruleForm.value = {} store.clearRuleForm() } const handleOpen = (status) => { switchKG.value = !status } const formFields = ref([]) const loadFormFields = async () => { let ObjectForm = await searchFormFn(props.keys) const setObj = (param) => { const keys = Object.keys(param) const Fields = keys.reduce((acc, key) => { if (ObjectForm[key] && ObjectForm[key].attributes) { const field = { ...ObjectForm[key] } field.attributes = { ...field.attributes, labelShow: props.show, labelWidth: props.width } acc.push(field) } return acc }, []) return Fields } if (Object.prototype.toString.call(ObjectForm) === '[object Object]') { if (Array.isArray(props.formData)) { formFields.value = props.formData.map((item) => ({ ...item, Options: setObj(item.Options) })) } else { formFields.value = [{ label: '', Options: setObj(props.formData) }] } } } watch( [() => props.keys, () => props.formData], async () => { await loadFormFields() }, { immediate: true } ) onMounted(() => { loadFormFields() })</script>
